...
function pushWrappedLine(lines: PdfLine[], text: string, options: Omit<PdfLine, 'text'> = {}, maxChars = 92) {
...
function tableRow(columns: Array<{ value: string; width: number; align?: 'left' | 'right' }>) {
  return columns.map((column) => pad(column.value, column.width, column.align)).join(' | ');
}
...
function buildReportLines(data: ReportsData) {
  ...
  pushWrappedLine(lines, `Generated: ${formatDateTime(generatedAt)} UTC`, { size: 10, gapBefore: 4 }, 100);
  ...
  pushWrappedLine(lines, `Materials tracked: ${data.inventorySummary.materialCount}`);
  pushWrappedLine(lines, `Transactions in range: ${data.activitySummary.totalTransactions}`);
  pushWrappedLine(lines, `Issued units in range: ${data.activitySummary.issueQuantity}`);
  pushWrappedLine(lines, `Receipts in range: ${data.activitySummary.receiveQuantity}`);

  lines.push({ text: 'Total Inventory by Material', size: 14, bold: true, gapBefore: 14 });
  lines.push({
    text: tableRow([
  ...
  lines.push({ text: 'Most Used Materials', size: 14, bold: true, gapBefore: 14 });
  lines.push({
    text: tableRow([
  ...
  if (data.topMaterials.length === 0) {
    pushWrappedLine(lines, 'No issue activity found for this date range.', { size: 10 });
  } else {
  ...
  lines.push({ text: 'Recent Activity Table', size: 14, bold: true, gapBefore: 14 });
  if (data.recentActivity.length === 0) {
    pushWrappedLine(lines, 'No recent transactions found for this date range.', { size: 10, gapBefore: 4 });
  } else {
    data.recentActivity.forEach((entry, index) => {
      lines.push({ text: `Activity ${index + 1}`, size: 11, bold: true, gapBefore: index === 0 ? 4 : 10 });
      pushWrappedLine(lines, `Date: ${formatDateTime(entry.createdAt)} UTC`, { size: 10 });
      pushWrappedLine(lines, `Type: ${entry.type}`, { size: 10 });
      pushWrappedLine(lines, `Material: ${entry.materialName} (${entry.materialSku})`, { size: 10 });
      pushWrappedLine(lines, `Quantity: ${entry.quantity} ${entry.unit}`, { size: 10 });
...
function buildPdfBuffer(lines: PdfLine[]) {
  ...
  for (const pageContent of pages) {
    const streamId = addObject(`<< /Length ${Buffer.byteLength(pageContent, 'utf8')} >>\nstream\n${pageContent}endstream`);
